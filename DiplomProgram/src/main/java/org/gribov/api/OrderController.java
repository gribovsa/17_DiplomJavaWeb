...
public class OrderController {
    ...
    public ResponseEntity<Order> issueBook(@RequestBody OrderRequest request) {
        log.info("Получен запрос на создание заказа: userId = {}, hydrobiontId = {}", request.getUserId(), request.getHydrobiontId());
     ...
     */
    @GetMapping
    public ResponseEntity<List<Order>> getAllIssue() {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "409", description = "Неверный ID заказа, заказ не найден",
                    content = @Content)})
    //--------------------------
    public ResponseEntity<Order> returnedIssue(@PathVariable Long id) {
